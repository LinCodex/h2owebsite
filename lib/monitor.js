...
function canNotify(stored, status) {
  ...
  if (!changed && !firstCheckDown) return false;

  const lastNotifiedAt = stored?.lastNotifiedAt
    ? new Date(stored.lastNotifiedAt).getTime()
    : 0;
  const cooldownActive = Date.now() - lastNotifiedAt < NOTIFY_COOLDOWN_MS;
...
async function runCheck({ notify = false } = {}) {
  ...
  if (notify && canNotify(stored, status)) {
    if (status === "down") {
      await sendNtfy(
        "H2O site is DOWN",
...
async function runTestCheck() {
  ...
  await sendNtfy(
    `H2O status check: ${label}`,
    `${SITE_URL} is ${label.toLowerCase()}.${http}${result.error ? ` ${result.error}` : ""}`,
    status === "up" ? "default" : "high"
  );
